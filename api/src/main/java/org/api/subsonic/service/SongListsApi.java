...
public class SongListsApi {
    ...
    public AlbumList2Res getAlbumList2(SubsonicCommonReq req, String type, Long size, Long offset, Long fromYear, Long toYear, Long genre, Long musicFolderId) {
        ...
        for (TbAlbumPojo albumPojo : albumList) {
            ...
            e.setStarred(LocalDateUtil.formatUTC(albumPojo.getPublishTime()));
            e.setSortName("");
            e.setUserRating(0);
    ...
    public RandomSongsRes getRandomSongs(SubsonicCommonReq req, Long size, String genre, Long fromYear, Long toYear, Long musicFolderId) {
        ...
        Map<Long, List<TbTagPojo>> labelMusicGenre = tagManagerService.getMusicGenre(musicIds);
        ...
        for (TbMusicPojo musicPojo : musicListByAlbumId) {
            ...
            e.setStarred(LocalDateUtil.formatUTC(musicPojo.getPublishTime()));
    ...
    public SongsByGenreRes getSongsByGenre(SubsonicCommonReq req, Long genre, Long musicFolderId, Long count, Long offset) {
        ...
        for (TbMusicPojo musicPojo : musicListByAlbumId) {
            ...
            e.setType("music");
            e.setIsVideo(false);
            e.setCreated(LocalDateUtil.formatUTC(musicPojo.getPublishTime()));
            song.add(e);
        }
    ...
    public StarredRes getStarred(SubsonicCommonReq req, Long musicFolderId) {
        ...
        if (CollUtil.isNotEmpty(albumIds)) {
            ...
            for (TbAlbumPojo tbAlbumPojo : tbAlbumPojos) {
                ...
                e.setCoverArt(StringUtil.defaultNullString(tbAlbumPojo.getId()));
                e.setCreated(LocalDateUtil.formatUTC(tbAlbumPojo.getCreateTime()));
                e.setDuration(DurationUtil.getDuration(albumDurationCount.get(tbAlbumPojo.getId())));
                e.setIsDir(true);
                e.setIsVideo(false);
                e.setPlayCount(0);
                e.setPlayed(LocalDateUtil.formatUTC(tbAlbumPojo.getPublishTime()));
                e.setSongCount(albumMusicCountByMapAlbumId.get(tbAlbumPojo.getId()));
                e.setStarred(LocalDateUtil.formatUTC(tbAlbumPojo.getPublishTime()));
        ...
        if (CollUtil.isNotEmpty(artistIds)) {
            ...
            for (TbArtistPojo tbArtistPojo : tbArtistPojos) {
                ...
                e.setStarred(LocalDateUtil.formatUTC(tbArtistPojo.getCreateTime()));
        ...
        if (CollUtil.isNotEmpty(userPlayList)) {
            ...
            for (TbMusicPojo musicPojo : tbMusicPojos) {
                ...
                e.setIsVideo(false);
                e.setCreated(LocalDateUtil.formatUTC(musicPojo.getCreateTime()));
                e.setStarred(LocalDateUtil.formatUTC(musicPojo.getCreateTime()));
    ...
    public Starred2Res getStarred2(SubsonicCommonReq req, Long musicFolderId) {
        ...
        if (CollUtil.isNotEmpty(albumIds)) {
            ...
            for (TbAlbumPojo tbAlbumPojo : tbAlbumPojos) {
                ...
                e.setIsVideo(false);
                e.setPlayCount(0);
                e.setPlayed(LocalDateUtil.formatUTC(tbAlbumPojo.getCreateTime()));
                e.setSongCount(albumMusicCountByMapAlbumId.get(tbAlbumPojo.getId()));
                e.setStarred(LocalDateUtil.formatUTC(tbAlbumPojo.getPublishTime()));
                e.setYear(LocalDateUtil.getYear(tbAlbumPojo.getPublishTime()));
