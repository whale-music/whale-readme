...
(
    100
) collate utf8_bin default '' null comment '字典记录实际数据',
    dict_value varchar
(
    100
) collate utf8_bin default '' null comment '字典键值',
    dict_type varchar
(
    100
) collate utf8_bin default '' null comment '字典类型',
...
(
    100
) collate utf8_bin default '' null comment '字典名称',
    dict_type varchar
(
    100
) collate utf8_bin default '' null comment '字典类型',
...
(
    128
) not null comment '登录用户名',
    nickname varchar
(
    128
) not null comment '登录显示昵称',
    password varchar
(
    20
) null comment '用户密码',
...
(
    512
) not null comment '专辑名',
    sub_type varchar
(
    128
) null comment '专辑版本（比如录音室版，现场版）',
...
(
    32
) null comment '保存音乐本体的md5，当上传新的音乐时做比较。如果相同则表示已存在',
    level char
(
    8
) null comment '音乐质量',
