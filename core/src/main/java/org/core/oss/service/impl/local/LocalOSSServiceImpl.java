...
//      */
//     @Override
//     public void isExist(String path, ResourceEnum type) {
...
//         try {
//             // 音乐地址URL缓存
//             FileMetadata item = MUSIC_PATH_CACHE.get(path);
...
//                 item = MUSIC_PATH_CACHE.get(path);
//             }
//             // 没有找到文件直接抛出异常
//             if (item == null || StringUtils.isBlank(item.getUri())) {
//                 throw new BaseException(ResultCode.DATA_NONE_FOUND.getCode(), ResultCode.DATA_NONE_FOUND.getResultMsg() + ": " + path);
//             }
//             return getPath(item);
...
//      */
//     @Override
//     public Set<String> getResourceToMapUrl(Collection<String> path, boolean refresh, ResourceEnum type) {
//         try {
//             // 音乐地址URL缓存
//             Set<FileMetadata> collect = path.parallelStream().map(MUSIC_PATH_CACHE::get).collect(Collectors.toSet());
//             // 没有地址便刷新缓存,获取所有文件保存到缓存中
//             // 第一次执行，必须刷新缓存。所以添加添加缓存是否存在条件
...
//                 // 更新初始化音乐数量
//                 this.initMusicAllCount = MUSIC_PATH_CACHE.size();
//                 collect = path.parallelStream().map(MUSIC_PATH_CACHE::get).collect(Collectors.toSet());
//             }
//             // 没有找到文件直接抛出异常
...
//      */
//     @Override
//     public String getResourceUrlByMd5(String md5, boolean refresh, ResourceEnum type) {
//         // 音乐地址URL缓存
//         // 没有地址便刷新缓存,获取所有文件保存到缓存中
//         // 第一次执行，必须刷新缓存。所以添加添加缓存是否存在条件
...
//      */
//     @Override
//     public Set<Resource> getResourceList(boolean refresh, ResourceEnum type) {
...
//      */
//     @Override
//     public Resource getResource(String path, boolean refresh, ResourceEnum type) {
//         // 音乐地址URL缓存
//         FileMetadata metadata = MUSIC_PATH_CACHE.get(path);
//         // 没有地址便刷新缓存,获取所有文件保存到缓存中
//         // 第一次执行，必须刷新缓存。所以添加添加缓存是否存在条件
...
//             // 更新初始化音乐数量
//             this.initMusicAllCount = MUSIC_PATH_CACHE.size();
//             metadata = MUSIC_PATH_CACHE.get(path);
//         }
...
//      */
//     @Override
//     public boolean delete(List<String> name, ResourceEnum type) {
//         try {
//             // 忽略无文件错误
//             isExist(name.get(0), );
//         } catch (BaseException e) {
