...
public class SyncPlayListMusicPlugin implements CommonPlugin {
    
    public static final String PlAY_LIST_ID_KEY = "musicId";
     ...
     */
    private void saveUserPlayListMusicList(PluginPackage pluginPackage, String playListId, String cookie, Long localUserId) {
        String playDetailStr = getPlayDetail(playListId, cookie);
        String playName = readJson(playDetailStr, "$.playlist.name");
        List<Long> playDetail = readJson(playDetailStr, "$.playlist.trackIds[*].id");
        ...
        for (MusicDetails tbMusicPojo : musicPojoList) {
            ...
            tbMusicPojo.getResource().forEach(tbResourcePojo -> {
                pluginPackage.logInfo("添加音乐下载地址：{}\tID:{}", tbResourcePojo.getPath(), tbResourcePojo.getId());
            });
    ...
    public MusicDetails saveMusicInfo(Map<Long, Map<String, Object>> songUrlMap, Map<String, Object> song, String cookie, Long userId) {
        ...
        audioMusic.setMusicName(MapUtil.getStr(song, "name"));
        JSONArray alia = MapUtil.get(song, "alia", JSONArray.class, new JSONArray());
        audioMusic.setAliaName(alia.toList(String.class));
        audioMusic.setTimeLength(MapUtil.getInt(song, "dt"));
        
        PicConvert audioMusicPic = new PicConvert();
        audioMusicPic.setPath(blurPicUrl);
        audioMusic.setPic(audioMusicPic);
        ...
        for (Object arItem : ar) {
            ...
            // 地区 只有城市编码
            artistPojo.setLocation(CityPlstMusicPojo.city.get(MapUtil.getStr(user, "city")));
            // 性别
        ...
        if (musicUrlMap != null && StringUtils.isNotBlank(url)) {
            ...
            // 上传md5值，或音乐文件
            audioSource.setPathTemp(dto.getUploadFlag() ? md5 + "." + type : url);
     ...
     * @param playId 歌单名
     */
    public String getPlayDetail(String playId, String cookie) {
        return req(host + "/playlist/detail?id=" + playId, cookie);
    }
    
    public <T> T readJson(String json, String jsonPath) {
        try {
